...
import { Search, Plus, TrendingUp, Building2, X, ChevronRight, LayoutGrid, List, FolderKanban } from 'lucide-react';
import { formatCurrency, formatDate, getInitials } from '@/lib/utils';
import { getClients, createClientAction, getProjetProspects, updateProjetProspectStatut } from './actions';
...
export default function ClientsPage() {
  ...
  const [projetProspects, setProjetProspects] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [formLoading, setFormLoading] = useState(false);

  const loadData = useCallback(async () => {
    const [clientsData, projetsData] = await Promise.all([
      getClients(),
      getProjetProspects(),
    ]);
    setClients(clientsData);
    setProjetProspects(projetsData);
    setLoading(false);
  }, []);
  ...
  const handleProjetStatutChange = async (id: string, statut: ProspectStatut) => {
    await updateProjetProspectStatut(id, statut);
    await loadData();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
        ...
        <div className="card p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-fuchsia-100 text-fuchsia-600 flex items-center justify-center shrink-0">
            <FolderKanban className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div className="min-w-0">
            <p className="text-lg sm:text-2xl font-bold text-gray-900">{projetProspects.length}</p>
            <p className="text-[10px] sm:text-xs text-gray-500 truncate">Projets en cours</p>
          </div>
        </div>
        <div className="card p-3 sm:p-4 flex items-center gap-2 sm:gap-3">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-1">
          ...
          <div className="flex items-center bg-gray-100 rounded-lg p-0.5 shrink-0">
            <button onClick={() => setView('kanban')} className={`p-2 rounded-md transition-colors ${view === 'kanban' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`} title="Pipeline projet">
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button onClick={() => setView('list')} className={`p-2 rounded-md transition-colors ${view === 'list' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`} title="Liste clients">
      ...
      {/* Content */}
      {view === 'kanban' ? (
        <ClientKanban prospects={projetProspects} onStatutChange={handleProjetStatutChange} />
      ) : (
        <>
          {filtered.length === 0 ? (
