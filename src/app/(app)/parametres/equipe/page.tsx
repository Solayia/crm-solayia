...
import { Plus, Shield, User, Trash2, Mail, Copy, Check, UserPlus, X } from 'lucide-react';
...
export default function EquipePage() {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {/* Invite form — modal on mobile, inline on desktop */}
      {showInvite && (
        <>
          {/* Mobile: bottom-sheet modal */}
          <div className="sm:hidden fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setShowInvite(false)}>
            <div className="card p-5 w-full rounded-b-none max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-gray-900">Inviter un nouveau membre</h3>
                <button onClick={() => setShowInvite(false)} className="p-2 text-gray-400 hover:text-gray-600 -mr-2"><X className="w-5 h-5" /></button>
              </div>
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} className="input-field pl-9" placeholder="email@exemple.fr" />
                  </div>
                </div>
              ...
              {inviteResult?.success && inviteResult.tempPassword && (
                <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div className="flex items-center gap-2 bg-white border border-green-200 rounded px-3 py-2">
                    ...
                    <button onClick={() => copyPassword(inviteResult.tempPassword!)} className="p-2 text-gray-400 hover:text-green-600 rounded hover:bg-green-50 min-h-[44px] min-w-[44px] flex items-center justify-center">
      ...
      <div className="card divide-y divide-gray-100">
        {profiles.map((profile) => (
          <div key={profile.id} className="p-4 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <div className="flex items-center gap-1">
                  ...
                  <select
                    ...
                    className={`text-xs font-medium px-2 py-1 rounded-full border-0 cursor-pointer min-h-[32px] ${
                ...
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-red-500 rounded hover:bg-red-50 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
            ...
            <p className="text-[11px] text-gray-400 mt-1 ml-[52px] sm:hidden">Depuis {formatDate(profile.created_at)}</p>
            <p className="text-xs text-gray-400 mt-1 ml-[52px] hidden sm:block">Depuis {formatDate(profile.created_at)}</p>
          </div>
        ))}
      </div>
